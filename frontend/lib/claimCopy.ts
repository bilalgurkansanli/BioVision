...
export interface ClaimBlock {
  ...
  caveat_tr: string;
  /**
   * The same limitation in one plain sentence, shown first.
   *
   * `caveat_tr` is thorough and it is the reason this product can be trusted;
   * it is also the densest paragraph on the page, and it sat directly under the
   * figures where a reader with no insurance vocabulary met it before anything
   * else. This is the version for that reader. The full text is one click away
   * and still outside the block's own disclosure.
   */
  caveat_lead_tr: string;
...
export const PREMIUM_BLOCK: ClaimBlock = {
  ...
  caveat_lead_tr: "Bu bölüm bir ödeme YAPILDIKTAN sonrasını anlatır — ödeme yapılıp yapılmayacağını söylemez. Rakamlar lira değil, yüzde.",
...
export const PAYOUT_BLOCK: ClaimBlock = {
  ...
  caveat_lead_tr: "Bu bölüm size ne ödeneceğini söylemiyor; yalnızca ödemenin üst sınırının hangi kurala göre kurulduğunu.",
...
export const WRITEOFF_BLOCK: ClaimBlock = {
  ...
  caveat_lead_tr: "Bu bölüm çizgilerin nerede olduğunu söyler, aracınızın hangi tarafta kaldığını söylemez. Ona eksper karar verir.",
...
export const STATES_BLOCK: ClaimBlock = {
  ...
  caveat_lead_tr: "Bu üç metin de aracın çizginin hangi tarafında olduğunu söylemez.",
...
/**
 * The six words a claimant cannot avoid, in the plainest Turkish that is still
 * true.
 *
 * These are not simplifications of the rules — the rules and their articles stay
 * exactly where they are. This is a glossary, and it exists because the page was
 * written for someone who already knew what "rayiç", "sovtaj" and "muafiyet"
 * mean, and almost nobody reading it an hour after a crash does.
 *
 * Each entry says what the word means and, where it matters, what it is NOT.
 * "Pert" in particular: people use it for any badly damaged car, and it has a
 * specific legal meaning that decides whether the vehicle can ever be driven
 * again.
 */
export const GLOSSARY: { term: string; plain_tr: string }[] = [
  {
    term: "Rayiç değer",
    plain_tr:
      "Aracınızın kaza günündeki piyasa değeri. Poliçeyi yaptırdığınız gündeki değil — kaza günündeki. Enflasyonda bu fark sizin lehinizedir.",
  },
  {
    term: "Pert (tam hasar)",
    plain_tr:
      "Aracın onarılamayacak kadar hasarlı sayılması. Günlük dilde her ağır hasara pert denir; hukuken ayrı bir şey, ve kararını sigorta eksperi verir. Pert olan araç hurdaya ayrılır ve bir daha trafiğe çıkamaz.",
  },
  {
    term: "Ağır hasar",
    plain_tr:
      "Pert değil ama ona yakın. Araç trafikten çekilir; yeni muayene ve trafik sigortasıyla yeniden tescil edilebilir. Ama bu kayıt aracın ruhsatına işler ve değer kaybı isteme hakkınızı bitirir.",
  },
  {
    term: "Muafiyet",
    plain_tr:
      "Poliçenizde yazıyorsa, her hasarda sizin cebinizden çıkan sabit tutar. Sigorta bunun üstünü karşılar. Poliçenizde yoksa sıfırdır.",
  },
  {
    term: "Sovtaj",
    plain_tr:
      "Hurdaya ayrılan aracın hâlâ eden parası. Pert hâlinde aracı sigortaya bırakırsanız bu konu açılmaz. Kendinize bırakırsanız bu tutar ödemeden düşülür ve miktarını eksper belirler.",
  },
  {
    term: "Basamak / kademe",
    plain_tr:
      "Kaç yıldır hasarsız gittiğinizi gösteren sıra numarası. Yukarı çıktıkça primi ucuzlatır; bir hasar ödemesi sizi bir basamak aşağı indirir.",
  },
  {
    term: "Eksper",
    plain_tr:
      "Hasarı yerinde inceleyip raporunu yazan bağımsız uzman. Aracınızın pert olup olmadığına ve onarımın kaça mal olacağına karar veren kişi odur — bu sistem değil.",
  },
];
