...
def test_every_damage_class_has_measured_numbers() -> None:
    ...
    With the building specialist removed (ADR-034) every class in the enum comes
    from VehiDE again, so this is once more an exact equality rather than an
    equality with an exclusion list.
    """
